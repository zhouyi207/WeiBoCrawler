        ...
        loop {
            match state {
                ...
                State::ReadingNegRes => {
                    ...
                    // 必须按**服务端选定的方法**继续握手（RFC 1928），不能要求 res.0 与本地
                    // `method` 变量一致：当协商包同时包含 NO_AUTH 与 USER/PASS 时，服务端可能
                    // 返回 0x00；若此处仍强制 res == UserPass，会误报 MethodMismatch。
                    if res.0 == AuthMethod::UserPass && self.proxy_auth.is_none() {
