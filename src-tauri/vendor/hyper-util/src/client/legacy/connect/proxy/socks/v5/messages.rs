...
impl NegotiationReq<'_> {
    pub fn write_to_buf(&self, buf: &mut BytesMut) -> Result<usize, SerializeError> {
        // 与 curl 一致：需要账号密码时同时声明 NO_AUTH(0x00) 与 USER/PASS(0x02)。
        // 仅发送 0x02 时，部分 SOCKS 服务端会回复 0xFF（无可接受方法），reqwest 报
        // "server does not support user/pass authentication"，而同一代理在 curl 下正常。
        let need = if *self.0 == AuthMethod::UserPass { 4 } else { 3 };
        if buf.capacity() - buf.len() < need {
            return Err(SerializeError::WouldOverflow);
        }

        buf.put_u8(0x05); // Version
        if *self.0 == AuthMethod::UserPass {
            buf.put_u8(0x02); // NMETHODS: two options (RFC 1928)
            buf.put_u8(AuthMethod::NoAuth as u8);
            buf.put_u8(AuthMethod::UserPass as u8);
        } else {
